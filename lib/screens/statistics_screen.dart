...
class _StatisticsScreenState extends State<StatisticsScreen> {
                ...
                return SliverList(
                  delegate: SliverChildListDelegate([
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                          child: _buildStatCard(
                            'Avg Words',
                            totalNotes > 0 ? (totalWords / totalNotes).round().toString() : '0',
                            Icons.analytics,
                            Colors.purple,
                          ),
                        ),
                      ...
                      Wrap(
                        ...
                        children: categoryStats.entries.map((entry) {
                          ...
                          return Container(
                            ...
                            decoration: BoxDecoration(
                              color: getCategoryColor(entry.key, index).withOpacity(0.1),
                              borderRadius: BorderRadius.circular(20),
                              border: Border.all(
                                color: getCategoryColor(entry.key, index).withOpacity(0.3),
                              ),
                            ),
                            child: Row(
                              ...
                              child: Row(
                                children: [
                                  Container(
                                    ...
                                    height: 8,
                                    decoration: BoxDecoration(
                                      color: Color(note.colorValue ?? 0xFFFFD54F),
                                      shape: BoxShape.circle,
                                    ),
                                  ),
                                  const SizedBox(width: 12),
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          _buildQuickStat('Notes created today', _getNotesCreatedToday().toString()),
                          const Divider(height: 24),
                          _buildQuickStat('Notes updated this week', _getNotesUpdatedThisWeek().toString()),
                          const Divider(height: 24),
                          _buildQuickStat('Longest note', '${_getLongestNote()} words'),
                          const Divider(height: 24),
                          _buildQuickStat('Most used category', _getMostUsedCategory()),
                        ],
                      ),
                    ),
  ...
  int _getLongestNote() {
    ...
    return notesBox.values.map((note) => note.wordCount).reduce((a, b) => a > b ? a : b);
  }
